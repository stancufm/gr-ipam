...
Session files are deliberately excluded from Git. They must be protected like credentials: restrict access, encrypt backups, define retention and deletion rules, and never attach them to public issues. Replay exposes captured stdin, including passwords typed with terminal echo disabled. Automated vault passwords passed through `sshpass` or injected by a session driver do not traverse recorded terminal stdin and are not copied into the audit file unless the remote side displays them.
